...
// Pipeline
include single_sample as SCANPY_SINGLE_SAMPLE from '../src/scanpy/main.nf' params(params)
...
workflow single_sample {
    ...
    main:
        ...
        SCANPY_SINGLE_SAMPLE( SC__FILE_CONVERTER.out )

        // Conversion
        // Convert h5ad to X (here we choose: loom format)
        if(params.sc.scanpy.containsKey("filter")) {
            filteredloom = SC__H5AD_TO_FILTERED_LOOM( SCANPY_SINGLE_SAMPLE.out.filtered_data )
            // In parameter exploration mode, this automatically merge all the results into the resulting loom
            scopeloom = FILE_CONVERTER(
                SCANPY_SINGLE_SAMPLE.out.final_processed_data.groupTuple(),
                'loom',
                SCANPY_SINGLE_SAMPLE.out.filtered_data
            )
        } else {
        ...
        finalH5ad = SC__PUBLISH_H5AD(
            SCANPY_SINGLE_SAMPLE.out.final_processed_data.map { 
