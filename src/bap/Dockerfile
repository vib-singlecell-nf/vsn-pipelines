...
# install BAP
RUN pip install biopython rapidfuzz fuzzysearch bap-atac

##################################################
# prepare R install
RUN R -e "install.packages( c('devtools','BiocManager', 'dplyr', 'data.table'))"
RUN R -e "BiocManager::install(c('BiocVersion', 'GenomicRanges'), update=TRUE, ask=FALSE)"
