...
parser.add_option("-s", "--var-subset",
                    action="store",
                    dest="var_subset",
                    default=None,
...
if options.method == 'combat':
    ...
    corrected = mnn_correct(*list(map(lambda adata: adata[:, adata.var.index.isin(hvgs)], adatas)),
