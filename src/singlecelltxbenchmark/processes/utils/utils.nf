...
process SC__FILE_CONVERTER {
  ...
  script:
    """
    python ../../../src/singlecelltxbenchmark/scripts/utils/sc_file_converter.py --input-format $params.iff --output-format $params.off $f "${id}.SC__FILE_CONVERTER.${params.off}"
    """
}
...
process SC__FILE_CONCATENATOR() {
  ...
  script:
    """
    python ../../../src/singlecelltxbenchmark/src/scripts/utils/sc_file_concatenator.py \
