...
workflow test_CISTARGET {
    ...
    main:
        ...
        motifDB = Channel
            .fromPath( params.getToolParams("scenic").cistarget.mtfDB )
            .collect() // use all files together in the ctx command
        motifANN = file(params.getToolParams("scenic").cistarget.mtfANN)
        ...
        */
        trackDB = Channel
            ...
            .collect() // use all files together in the ctx command
        trackANN = file(params.getToolParams("scenic").cistarget.trkANN)
...
workflow test_SINGLE_RUN_BY_ID {
    ...
    main:
        filteredloom = file( params.getToolParams("scenic").filteredloom )
        tfs = file(params.getToolParams("scenic").grn.TFs)
...
workflow {
    ...
    main:
        switch(params.test) {
            ...
            break;
            case "CISTARGET":
                grn = Channel.fromPath(params.getToolParams("scenic").scenicoutdir + "/grnboost2withoutDask/run_*/run_*__adj.tsv")
                test_CISTARGET( file( params.getToolParams("scenic").filteredloom ), grn )
            break;
            case "AUCELL":
                ctx_mtf = Channel.fromPath(params.getToolParams("scenic").scenicoutdir + "/cistarget/run_*/run_*__reg_mtf.csv")
                ctx_trk = Channel.fromPath(params.getToolParams("scenic").scenicoutdir + "/cistarget/run_*/run_*__reg_trk.csv")
                test_AUCELL( file( params.getToolParams("scenic").filteredloom ), ctx_mtf, ctx_trk )
            break;
            case "AGGR_MULTI_RUNS_FEATURES":
                /* Aggregate motifs from multiple runs */
                reg_mtf = Channel.fromPath(params.getToolParams("scenic").scenicoutdir + "/cistarget/run_*/run_*__reg_mtf.csv")
                ...
                if(params.getToolParams("scenic").cistarget.trkDB) {
                    ...
                    reg_trk = Channel.fromPath(params.getToolParams("scenic").scenicoutdir + "/cistarget/run_*/run_*__reg_trk.csv")
                    AGGR_MULTI_RUNS_FEATURES__TRACK( reg_trk.collect(), 'trk' )
                }
            break;
            case "AGGR_MULTI_RUNS_REGULONS":
                /* Aggregate motif regulons from multiple runs */
                auc_mtf_looms = Channel.fromPath(params.getToolParams("scenic").scenicoutdir + "/aucell/run_*/run_*__auc_mtf.loom")
                AGGR_MULTI_RUNS_REGULONS__MOTIF( auc_mtf_looms.collect(), 'mtf' )
                if(params.getToolParams("scenic").cistarget.trkDB) {
                    /* Aggregate track regulons from multiple runs */
                    auc_trk_looms = Channel.fromPath(params.getToolParams("scenic").scenicoutdir + "/aucell/run_*/run_*__auc_trk.loom")
                    AGGR_MULTI_RUNS_REGULONS__TRACK( auc_trk_looms.collect(), 'trk' )
                }
            break;
            case "AUCELL_FROM_FOLDER":
                /* Aggregate motif regulons from multiple runs */
                regulons_folder_mtf = file(params.getToolParams("scenic").scenicoutdir + "/multi_runs_regulons_mtf")
                AUCELL_FROM_FOLDER__MOTIF( file(params.getToolParams("scenic").filteredloom), regulons_folder_mtf, 'mtf' )
                if(params.getToolParams("scenic").cistarget.trkDB) {
                    /* Aggregate track regulons from multiple runs */
                    regulons_folder_trk = file(params.getToolParams("scenic").scenicoutdir + "/multi_runs_regulons_trk")
                    AUCELL_FROM_FOLDER__TRACK( file(params.getToolParams("scenic").filteredloom), regulons_folder_trk, 'trk' )
                }
            break;
            case "SAVE_SCENIC_MULTI_RUNS_TO_LOOM_MOTIF":
                filteredloom = file(params.getToolParams("scenic").filteredloom)
                aggr_features_mtf = file(params.getToolParams("scenic").scenicoutdir + "/multi_runs_cistarget/multi_runs_features_mtf.csv.gz")
                regulons_folder_mtf = file(params.getToolParams("scenic").scenicoutdir + "/multi_runs_regulons_mtf")
                regulons_auc_mtf = file(params.getToolParams("scenic").scenicoutdir + "/multi_runs_aucell/multi_runs_regulons_auc_mtf.tsv")
            ...
            case "SAVE_SCENIC_MULTI_RUNS_TO_LOOM_TRACK":
                filteredloom = file(params.getToolParams("scenic").filteredloom)
                regulons_folder_trk = file(params.getToolParams("scenic").scenicoutdir + "/multi_runs_regulons_trk")
                aggr_features_trk = file(params.getToolParams("scenic").scenicoutdir + "/multi_runs_cistarget/multi_runs_features_trk.csv.gz")
                regulons_auc_trk = file(params.getToolParams("scenic").scenicoutdir + "/multi_runs_aucell/multi_runs_regulons_auc_trk.tsv")
            ...
            case "MERGE_MOTIF_TRACK_LOOMS":
                scenic_loom_mtf = file( params.getToolParams("scenic").scenicoutdir + "/multi_runs_looms/multi_runs_regulons_auc_mtf.loom" )
                scenic_loom_trk = file( params.getToolParams("scenic").scenicoutdir + "/multi_runs_looms/multi_runs_regulons_auc_trk.loom" )
                MERGE_MOTIF_TRACK_LOOMS(
                    scenic_loom_mtf,
                    scenic_loom_trk
                )
            ...
            case "VISUALIZE_PUBLISH":
                /* Aggregate motif regulons from multiple runs */
                scenic_loom = file( params.getToolParams("scenic").scenicoutdir + "/" + params.getToolParams("scenic").scenicOutputLoom )
