...
parser_grn.add_argument('motif_enrichment_table_fname',
                        type=argparse.FileType('r'),
...
parser_grn.add_argument('auc_mtx_fname',
                        type=argparse.FileType('r'),
...
parser_grn.add_argument('--min-genes-regulon',
                        type=int, default=5,
...
regulons = list(map(lambda x: x.copy(gene2occurrence=list(filter(lambda y: y.name == x.name, signatures))[0].gene2weight), regulons))
...
# Create loom
export2loom(ex_mtx=ex_matrix_df,
            regulons=regulons,
