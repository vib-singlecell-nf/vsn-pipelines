...
{
  o <- order(cell.counts$V1, decreasing = TRUE)
  stuff <- rle(cell.counts[[1]])
  ...
  out <- DataFrame(rank = .reorder(run.rank, stuff$lengths, 
...
cell.counts <- read.table(file = args[1], header = F, sep = "\t", quote = '', stringsAsFactors = F)
...
x_knee <- br.out.df %>% 
  filter(total > br.out@metadata$knee) %>% 
  arrange(total) %>% 
  select(rank) %>% 
  head(1)
x_inflection <- br.out.df %>% filter(total > br.out@metadata$inflection) %>% arrange(total) %>% select(rank) %>% head(1)
padding <- length(br.out$rank) / 10

# Plot the Barcode Rank vs Total UMI
print("Plot the Barcode Rank vs Total UMI")
plot <- ggplot(br.out.df, aes(x = rank, y = total)) +
  geom_point() +
  scale_x_log10() +
  scale_y_log10() +
  theme_bw() +
  ...
  geom_hline(yintercept = br.out@metadata$knee, linetype = 2, colour = "dodgerblue") +
  geom_hline(yintercept = br.out@metadata$inflection, linetype = 2, colour = "forestgreen") +
  labs(x = "Rank", y = "Total", title = "Barcode Rank vs Total UMI") +
  annotate("text", label = paste0("Knee (", x_knee, ")"), x = x_knee$rank + padding, y = br.out@metadata$knee, size = 5) +
  annotate("text", label = paste0("Inflection (", x_inflection, ")"), x = x_inflection$rank + padding, y = br.out@metadata$inflection, size = 5)
ggsave(filename = paste0(args[2], ".barcode_rank_vs_total_umi_plot.png"), plot = plot, device = "png", width = 10, height = 10, type = "cairo")

# Saving selected cell barcodes
print("Save selected cell barcodes")
write.table(x = cell.counts[1:x_knee$rank, "V2"], file = paste0(args[2],".selected_cell_barcodes_by_knee.txt"), quote = F, sep = "\t", row.names = F, col.names = F)
write.table(x = cell.counts[1:x_inflection$rank, "V2"], file = paste0(args[2],".selected_cell_barcodes_by_inflection.txt"), quote = F, sep = "\t", row.names = F, col.names = F)