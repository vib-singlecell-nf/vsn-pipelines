...
from pyscenic.utils import COLUMN_NAME_MOTIF_SIMILARITY_QVALUE, COLUMN_NAME_ORTHOLOGOUS_IDENTITY, \
    COLUMN_NAME_MOTIF_ID, COLUMN_NAME_TF, COLUMN_NAME_ANNOTATION, \
    ACTIVATING_MODULE, REPRESSING_MODULE
from pyscenic.transform import COLUMN_NAME_NES, COLUMN_NAME_TYPE, COLUMN_NAME_CONTEXT, COLUMN_NAME_TARGET_GENES
import re
import sys
import utils
...
parser_grn = argparse.ArgumentParser(usage="usage: %prog [options] feature_enrichment_table_fnames",
                                     description='Aggregate feature (motif or track) enrichment tables from multiple SCENIC runs. ')

parser_grn.add_argument('feature_enrichment_table_fnames',
                        nargs='+',
...
def stack_feature_enrichment_tables(feature_enrichment_table_fnames):
    ...
    for i in range(0, len(feature_enrichment_table_fnames)):
        ...
        # Add the run ID
        feature_enrichment_table[('', 'RunID')] = int(re.search(r'run_([0-9]+)+__reg_(mtf|trk)\.csv$', str(fname)).group(1))
        ...
        if multi_runs_feature_enrichment_table is None:
            ...
            multi_runs_feature_enrichment_table = pd.concat([multi_runs_feature_enrichment_table, feature_enrichment_table])
